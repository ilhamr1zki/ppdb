<?php  
	...
	// Cek apakah akses membawa token
	if ($_GET['id'] == false) {
	...
	// check token
	$queryToken = mysqli_query($con, "
		SELECT is_token FROM token WHERE is_token = '$_GET[id]'
	");

	$findToken = mysqli_num_rows($queryToken);
	...
	if ($findToken != 1) {
		...
		// check apakah sesi sudah expired
		$queryCheckSession = mysqli_query($con, "
			SELECT is_expired FROM token
			WHERE is_token = '$_GET[id]'
		");

		$getSession = mysqli_fetch_assoc($queryCheckSession);

		if ($getSession['is_expired'] == 0) {
		...
		} else if ($getSession['is_expired'] == 1) {
...
<!DOCTYPE html>
	...
	<body background="./assets/images/backgrounds/symphony.png">
		...
		<form action="status" method="post" style="background-color: white;">
			<input type="hidden" name="form_token" value="<?= $token; ?>">
			<input type="hidden" name="is_token" value="<?= $_GET['id']; ?>">
			<button type="submit" name="send" id="send" style="border: 1px solid white;"></button>
		</form>
							...
							<div class="panel-body">
								...
								<?php else: ?>
									...
									<div class="form-group">
										<!-- <label class="col-md-3 control-label"> PENDAFTARAN UNTUK KELAS<sup style="color: red;">*</sup> : </label> -->
										<div class="col-md-12">
											<center> <h1 id="processing_data"> SEDANG PROSES DATA </h1> </center>
										</div>
									</div>
